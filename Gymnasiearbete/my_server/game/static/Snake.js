class Snake {
    ...
    registerInput() {
        document.addEventListener('keydown', (event) => {
            // Convert to lowercase to handle 'W' or 'w' equally
            let key = event.key.toLowerCase(); 
            let newDir = null;

            // Map standard keys to direction vectors [x, y]
            // Note: In canvas/grid systems, y goes down as it increases.
            if (key === 'a') {
                newDir = [-1, 0];  // Left
            } else if (key === 'd') {
                newDir = [1, 0];   // Right
            ...
            // If a valid movement key was pressed, send the intent to the Python server
            if (newDir !== null) {
                this.socket.emit('change_direction', { dir: newDir });
            }
        }, false);
    }
